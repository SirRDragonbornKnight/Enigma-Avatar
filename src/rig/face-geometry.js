...
const perp = (u) => {
  // any unit vector ⊥ u — used when the caller's forward ∥ up
...
export function analyzeMorphGeometry(model, opts = {}) {
  ...
  const p = {
    ...
    band: opts.band ?? DEF.band,
    supportRel: opts.supportRel ?? DEF.supportRel,
  ...
  model.traverse((o) => {
    ...
    for (let mi = 0; mi < targets.length; mi++) {
      ...
      const floor = Math.max(maxLen * p.supportRel, span * 1e-6),
        split = SPLIT * span;
      ...
      const mu = r.Su / r.M,
        cov = r.Shu / r.M - r.ch * mu,
        varH = Math.max(0, r.Shh / r.M - r.ch * r.ch);
      ...
      if (
        r.wL > 0 &&
        r.wR > 0 &&
        Math.min(r.wL, r.wR) / Math.max(r.wL, r.wR) >= LOBE_BAL &&
        r.wC / r.M < CENTER_MAX &&
        Math.min(-r.sxL / r.wL, r.sxR / r.wR) >= LOBE_MIN * span
      )
        r.mirrored = true;
    }
  });
  ...
  for (const r of recs) {
    ...
    out.morphs.push({
      index: r.index,
      meshName: r.meshName,
      support: r.support, // support = SAMPLED support vert count
      ...
      eyeScore: r.eyeScore,
      mouthScore: r.mouthScore,
      mirrored: r.mirrored,
    });
    const s = out.byIndex.get(r.index);
    if (!s) out.byIndex.set(r.index, { eyeScore: r.eyeScore, mouthScore: r.mouthScore });
    else {
      s.eyeScore = Math.max(s.eyeScore, r.eyeScore);
      s.mouthScore = Math.max(s.mouthScore, r.mouthScore);
    }
  }
  const rank = (key) =>
    [...out.byIndex.entries()]
