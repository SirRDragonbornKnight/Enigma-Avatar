...
This pulls the *renderable* bits out into a flat folder under ``models/``:
  ...
  * **model mode (default)** — the mesh(es) (``.fbx/.gltf/.glb``) plus all
...
# HONEST coverage: only formats the overlay loader can parse (FBXLoader + GLTFLoader).
# There is NO OBJLoader/ColladaLoader, so .obj/.dae are intentionally excluded — importing
# one would produce a model that dies with a misleading "not valid JSON" on load.
MESH_EXT = {".fbx", ".gltf", ".glb"}
...
def extract_model(pkg_path: str, out_dir: str) -> dict:
    ...
    # Pick the primary mesh: prefer .fbx/.glb/.gltf by rank (the only loadable formats),
    ...
    def mesh_key(b: str):
        ...
        rank = {".fbx": 0, ".glb": 1, ".gltf": 2}.get(e, 9)
