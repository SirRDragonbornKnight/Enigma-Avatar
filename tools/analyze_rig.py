...
def world(i):
    if i in W:
        return W[i]
    m = local(nodes[i])
    W[i] = (world(parent[i]) @ m) if i in parent else m
    return W[i]
...
def report(title, pat):
    ...
    for i in hits:
        ...
        if kids:
            ...
            if n > 1e-6:
                v = v / n
                d = f" points=({v[0]:+.2f},{v[1]:+.2f},{v[2]:+.2f}) len={n:.3f}"
        print(
            f"  {name(i):42s} pos=({p[0]:+.2f},{p[1]:+.2f},{p[2]:+.2f}) parent={name(par) if par is not None else '-':22s}{d}"
        )
