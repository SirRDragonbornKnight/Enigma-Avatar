"""test_battery.py — the full-system live battery. Drives the RUNNING overlay over the bus:
  ...
  2) runs the behavior battery on a chosen model (gestures, emotes, outfit round-trip,
     bone naming/highlight, recolor, mesh toggle, impulse, physics ball, movement walls,
     monitor hop, stillness hash)
    ...
    for m in discover_models():
        ...
        if ok:
            ...
            row.update({
                ...
                "deforming": w.get("deforming"), "unsprungTwins": w.get("unsprungTwinBones"),
                "mouth": facial.get("mode"), "face": (facial.get("info") or "")[:90],
        ...
        rows.append(row)
        print(("OK   " if ok else "FAIL ") + m["id"] + (f"  roles={row.get('roles')} springs={row.get('springs')} deform={row.get('deforming')} mouth={row.get('mouth')} knee={row.get('knee')}" if ok else ""))
    return rows

async def behavior(bus, url):
    r = {}
    await bus.cmd({"action": "load", "url": url})
    await wait_model(bus, url); await asyncio.sleep(2)
    # stillness: two snaps 6s apart must hash identical (cursor untouched)
    a = await bus.snap("bat_still_a.png"); await asyncio.sleep(6); b = await bus.snap("bat_still_b.png")
    ...
    # gestures end-to-end (each returns to neutral)
    for g, wait in [("clap", 2.5), ("jump", 1.6), ("flip", 1.8), ("sit", 1.6), ("getup", 1.6), ("laydown", 2.0), ("getup", 1.8)]:
        await bus.cmd({"action": "gesture", "name": g}); await asyncio.sleep(wait)
    j = await bus.query("joints") or {}
    r["gestures"] = f"end: knee={j.get('leftKnee')} elbow={j.get('leftElbow')} gesturing={j.get('gesturing')}"
    for e in ["happy", "nod", "wag"]:
        await bus.cmd({"action": "express", "name": e}); await asyncio.sleep(0.8)
    r["emotes"] = "fired"
    # look + highlight + naming
    await bus.cmd({"action": "lookAt", "px": 100, "py": 100}); await asyncio.sleep(0.6)
    ...
    if bn: await bus.ask({"action": "nameBone", "bone": bn, "label": ""})    # clean up the label
    # outfit round-trip
    ...
    # physics ball
    await bus.cmd({"action": "gesture", "name": "throwball"}); await asyncio.sleep(4)
    r["ball"] = await bus.snap("bat_ball.png")
    await bus.cmd({"action": "gesture", "name": "clearballs"}); await asyncio.sleep(0.5)
    # movement walls: bottom + side, then recenter
    st = await bus.query("state") or {}
    sw, sh = (st.get("screen") or [2560, 1440])
    await bus.cmd({"action": "moveTo", "px": sw / 2, "py": sh + 500}); await asyncio.sleep(2)
    w1 = await bus.query("where") or {}
    await bus.cmd({"action": "moveTo", "px": -500, "py": sh * 0.6}); await asyncio.sleep(2)
    ...
    await bus.cmd({"action": "monitor", "value": "next"}); await asyncio.sleep(2.5)
    r["peerSnap"] = await bus.snap("bat_peer.png")
    await bus.cmd({"action": "monitor", "value": "prev"}); await asyncio.sleep(1.5)
    await bus.cmd({"action": "goTo", "to": "center"})
    # anim graceful failure (no clips bundled)
    r["animGraceful"] = await bus.ask({"action": "anim", "name": "definitely-missing"})
    return r
