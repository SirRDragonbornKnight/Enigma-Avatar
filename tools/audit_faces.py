...
OPEN_RE = re.compile(
    r"jaw.?open|mouth.?open|mouthopen|(^|[._-])aa($|[._-])|vrc\.v_aa|viseme.?aa|fcl[._-]?mth[._-]?a$|(^|[._-])v[._-]open|あ",
    re.I,
)
...
def load_gltf(path):
    ...
    if path.lower().endswith(".glb"):
        ...
        while off + 8 <= min(length, len(data)):
            ...
            off += clen
            if ctype == 0x4E4F534A:
                gjson = json.loads(chunk.decode("utf-8", "replace"))
        ...
        for b in gjson.get("buffers") or [{}]:
            uri = b.get("uri")
            if uri is None:
                bufs.append(bins[0] if bins else b"")
...
def read_accessor(g, bufs, ai):
    ...
    if sp:
        ...
        vals = (
            np.frombuffer(bufs[vbv.get("buffer", 0)], dtype=dt, count=cnt * ncomp, offset=voff)
            .reshape(cnt, ncomp)
            .astype(np.float32)
        )
        out = out.copy()
...
def morph_names_for(g, mesh):
    ...
    if names and len(names) >= nt:
        return list(names[:nt])
    return [None] * nt
...
def morph_features(g, bufs, mesh):
    ...
    for t in targets:
        if "POSITION" not in t:
            feats.append({"affectedFrac": 0.0, "note": "no POSITION delta"})
            continue
        try:
            d = read_accessor(g, bufs, t["POSITION"])
        except Exception as e:
            feats.append({"error": repr(e)})
            continue
        mag = np.linalg.norm(d, axis=1)
        mx = float(mag.max()) if len(mag) else 0.0
        if mx <= 1e-9:
            feats.append({"affectedFrac": 0.0, "note": "zero delta"})
            continue
        ...
        feats.append(
            {
                ...
                "centroidFrac": [
                    round(float((c[i] - lo[i]) / size[i]), 3) for i in range(3)
                ],  # x,y,z in 0..1 of mesh bbox
...
def guess_region(f, mesh_is_facey):
    ...
    if not f or f.get("error") or f.get("affectedFrac", 0) <= 0:
        return ""
...
def mine_weight_tracks(g, bufs):
    ...
    for ai, anim in enumerate(g.get("animations") or []):
        ...
        for ch in anim.get("channels") or []:
            ...
            if len(times) < 2 or vals.shape[0] != len(times):
                continue
            ...
            for k in range(nm):
                ...
                for t, x in zip(times, v):
                    if not inp and x >= hi:
                        ...
                        inp = False
                        pulses.append(float(t - t0))
                mean_on = float(v[v > lo].mean()) if (v > lo).any() else 0.0
                duty = float((v > lo).mean())
                pat = (
                    "blink-like"
                    if pulses and max(pulses) < 0.6 and len(pulses) >= 1 and duty < 0.5
                    else "sustained"
                    if duty > 0.5
                    else "pulsed"
                )
                tracks.append(
                    {
                        ...
                        "duty": round(duty, 3),
                        "pulses": len(pulses),
                        "pulseWidths": [round(p, 2) for p in pulses[:8]],
                        "meanOn": round(mean_on, 3),
                        "pattern": pat,
                    }
                )
            if tracks:
                out.append(
                    {
                        "anim": anim.get("name", f"<anim {ai}>"),
                        "mesh": mesh_i,
                        "meshName": g["meshes"][mesh_i].get("name", ""),
                        "dur": round(dur, 2),
                        "keyframes": len(times),
                        "tracks": tracks,
                    }
...
def audit(path):
    ...
    r["eyeBones"] = [
        n
        for _j, n in names
        if EYE_RE.search(n) and not EYE_NOT.search(n) and not LID_RE.search(n) and not BROW_RE.search(n)
    ]
    ...
    for mi, m in enumerate(meshes):
        ...
        r["meshMorphs"].append(
            {"mesh": mi, "meshName": mesh_name, "count": len(mnames), "named": len(named), "morphs": entries}
        )
    ...
    if r["vrm"]:
        blink_mode = "vrm"
    elif r["blinkNameHits"]:
        blink_mode = "morph"
    elif r["lidBones"]:
        blink_mode = "bones"
    elif total_morphs:
        blink_mode = "morph-geom?"
    else:
        blink_mode = "none"
...
def main():
    ...
    for d in sorted(os.listdir(base)):
        ...
        if not os.path.isdir(full):
            continue
        cand = [
            os.path.join(dp, f) for dp, _dn, fn in os.walk(full) for f in fn if f.lower().endswith((".glb", ".gltf"))
        ]
        if not cand:
            continue
    ...
    for r in results:
        ...
        if "error" in r:
            ...
            continue
        print(
            f"  mouth ladder: {r['mouthMode']:12s} blink ladder: {r['blinkMode']:12s} vrm={r['vrm']}  anims={r['animCount']}  morphs={r['totalMorphs']} ({r['namedMorphs']} named)"
        )
        print(
            f"  bones: jaw=[{fmt_list(r['jawBones'])}] lids={len(r['lidBones'])} eyes=[{fmt_list(r['eyeBones'])}] brows={len(r['browBones'])}"
        )
        ...
        for wt in r["weightTracks"]:
            ...
            for t in wt["tracks"]:
                print(
                    f"      morph #{t['morph']}: max={t['max']} duty={t['duty']} pulses={t['pulses']}{t['pulseWidths']} -> {t['pattern']}"
                )
