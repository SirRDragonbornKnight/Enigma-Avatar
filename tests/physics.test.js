...
test("rapier: a dropped ball falls, bounces, and settles on the floor", async () => {
  ...
  assert.ok(
    Math.abs(t.y - (floorY + r)) < 0.05,
    `at rest ON the floor line (y ${t.y.toFixed(3)} ≈ ${(floorY + r).toFixed(3)})`
  );
...
test("rapier: a ball dropped onto HER body capsule hits it and deflects off (she's a solid rounded body)", async () => {
  ...
  for (let i = 0; i < 120 * 5; i++) {
    ...
    if (Math.abs(t.x) < 0.3 && t.y < -0.95 && t.y > -3.0) tunneled = true; // passed straight DOWN through the capsule's volume
  }
  ...
  assert.ok(
    Math.abs(ball.translation().x) > 0.3,
    "the off-centre drop ROLLED OFF her body to the side (a rounded solid), didn't pass through"
  );
});
...
test("physics: a fast ball cannot tunnel a thin platform — CCD lands it on top", async () => {
  ...
  assert.ok(
    minObjY > topY - r - 0.05,
    `never tunneled below the slab top (minY ${minObjY.toFixed(3)} > ${(topY - r).toFixed(3)})`
  );
  assert.ok(
    Math.abs(restY - (topY + r)) < 0.06,
    `rests ON the platform top (y ${restY.toFixed(3)} ~= ${(topY + r).toFixed(3)})`
  );
});
...
test("physics: overlapping props separate, and clearProps leaves no dangling bodies", async () => {
  ...
  const r = 0.2,
    floorY = -3; // sizeWorld 0.4 → radius 0.2
  physics.setFloor(floorY);
  await physics.throwProp("ball.glb", { x: 0.0, y: 0 }, { x: 0, y: 0 }, 0.4);
...
test("GUARD: step() floors a NaN/<=0 dt — a thrown prop stays finite", async () => {
  ...
  assert.ok(
    ...
    `prop position stays finite through NaN/0/negative timesteps (${p.x},${p.y},${p.z})`
  );
});
