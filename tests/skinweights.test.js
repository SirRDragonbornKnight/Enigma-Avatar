...
import { computeWeightMass, subtreeMass, coincident, findRoleTwins, groupCoincidentRoots } from "../src/rig/skinweights.js";
...
function parallelLegRig() {
  ...
  const oT = bone("A_thigh", [0.2, 0, 0]),
    oS = bone("A_shin", [0, -0.8, 0]),
    oF = bone("A_foot", [0, -0.8, 0]);
  oT.add(oS);
  oS.add(oF);
  root.add(oT);
  const dT = bone("B_thigh", [0.2, 0, 0]),
    dS = bone("B_shin", [0, -0.8, 0]),
    dF = bone("B_foot", [0, -0.8, 0]);
  dT.add(dS);
  dS.add(dF);
  root.add(dT);
...
test("computeWeightMass reads real per-bone influence; control bones read zero", () => {
  ...
  assert.ok(
    Math.abs(mass.get(r.oT) - 1.8) < 1e-6 && Math.abs(mass.get(r.dT) - 1.2) < 1e-6,
    "exact masses (3 verts x weight)"
  );
...
test("coincident: same head + same direction = twin; offset or rotated = not", () => {
  ...
  const rot = bone("Rotated", [0.2, 0, 0]);
  rot.add(bone("RotChild", [0.8, 0, 0])); // same head, direction 90° off
  r.root.add(rot);
...
test("findRoleTwins finds every stranded deforming twin; adoption makes it idempotent", () => {
  ...
  assert.ok(
    r.dT.getWorldPosition(new THREE.Vector3()).distanceTo(before) < 1e-6,
    "attach preserved the world position"
  );
