...
export class StubColor {
  ...
  set(h) {
    this.hex = String(h).replace(/^#/, "");
    return this;
  }
  clone() {
    return new StubColor(this.hex);
  }
  copy(o) {
    this.hex = o.hex;
    return this;
...
export function makeApi(over = {}) {
  ...
  const api = {
        ...
        return [
          {
            id: "roxanne_wolf",
            label: "Roxanne Wolf",
            url: "./models/roxanne_wolf/scene.gltf",
            builtin: false,
            thumb: "./models/roxanne_wolf/.thumb.png",
          },
          { id: "c_za", label: "C za", url: "./models/c_za/c_za.glb", builtin: false, thumb: null },
        ];
      },
      importModel: async () => {
        calls.push(["importModel"]);
        return null;
      },
      // add-prop flow (a SHELL hard-req): native picker → copied into props/ → returns a url
      // the UI then hands to api.attachMesh. Returns a url so the importProp -> attachMesh
      // handoff is exercised (override with () => null to cover the cancel path).
      importProp: async () => {
        calls.push(["importProp"]);
        return { url: "./props/p_imported/p.glb" };
      },
      renameModel: async (id, label) => {
        calls.push(["renameModel", id, label]);
    ...
    getAttachObjs: () => [],
    getBonesShown: () => false,
    loadModel: rec("loadModel"),
    attachMesh: rec("attachMesh"),
    detachAttachment: rec("detachAttachment"),
    clearAttachments: rec("clearAttachments"),
    ball: rec("ball"),
    showSkeleton: rec("showSkeleton"),
    getShadowOn: () => true,
    setShadowOn: rec("setShadowOn"),
    recolor: rec("recolor"),
    hueShift: rec("hueShift"),
    springTune: rec("springTune"),
    tuneAttachment: rec("tuneAttachment"),
    resetColors: rec("resetColors"),
    materials: () => [
    ...
    setMeshVisible: rec("setMeshVisible"),
    setMeshLabel: rec("setMeshLabel"),
    ...
    diagnoseModel: async (id) => {
      calls.push(["diagnoseModel", id]);
      return { nodes: 200, mojibake: 5, recoverable: 0, names: ["Spine", "L_fluffShoulder", "L_elbow"] };
    },
    repairModel: async (opts) => {
      calls.push(["repairModel", opts]);
      return { ok: true, id: "m_fixed", url: "./models/m_fixed/m.glb", label: "M fixed", renamed: 1, repaired: 0 };
    },
    setYaw: rec("setYaw"),
    getYaw: () => 0,
