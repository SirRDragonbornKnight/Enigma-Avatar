...
const blink = () =>
  gridDelta((x, y) => Math.abs(x) >= 0.149 && Math.abs(x) <= 0.351 && y >= 1.57 && y <= 1.67, 0, -0.05, 0); // two lateral lobes at eye height, moving DOWN
...
test("one-sided wink pair earns `mirrored` through TWIN detection; a lone half does not", () => {
  ...
  assert.ok(
    rp.morphs.every((m) => m.mirrored),
    "both wink halves marked mirrored via the twin"
  );
...
test("no head bone → empty rankings, never a throw", () => {
  ...
  const r = analyzeMorphGeometry(root, {
    head: null,
...
test("byIndex merges same-index morphs across meshes by MAX; bad/zero meshes are skipped", () => {
  ...
  const r = analyzeMorphGeometry(root, OPTS(head));
  assert.deepEqual(
    r.morphs.map((m) => m.meshName).sort(),
    ["face", "lash"],
    "broken + bare meshes contribute no entries"
  );
  ...
  assert.ok(
    r.byIndex.size === 1 && r.byIndex.get(0).eyeScore > 0.5,
    "merged logical morph keeps the face mesh's MAX score"
  );
