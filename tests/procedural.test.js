...
test("squat-bound legs are normalized to standing at build; straight rigs untouched", () => {
  const ang = (model, names) => {
    ...
    const p = {};
    model.traverse((o) => {
      if (o.isBone && names.includes(o.name)) p[o.name] = o.getWorldPosition(new THREE.Vector3());
    });
    const v1 = p[names[0]].clone().sub(p[names[1]]),
      v2 = p[names[2]].clone().sub(p[names[1]]);
  ...
  sq.traverse((o) => {
    if (!o.isBone) return;
    if (/^(Left|Right)Thigh$/.test(o.name))
      o.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), -1.2));
    if (/^(Left|Right)Shin$/.test(o.name))
      o.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), 1.9));
  });
  ...
  sq.traverse((o) => {
    if (!o.isBone) return;
    if (o.name === "LeftThigh") th = o;
    if (o.name === "LeftShin") sh = o;
  });
  const a2 = new THREE.Vector3(),
    b2 = new THREE.Vector3();
  th.getWorldPosition(a2);
  sh.getWorldPosition(b2);
...
test("squat normalization fixes the leg TWIST: kneecaps track the toes, toes keep the bind heading (knock-knee)", () => {
  ...
  for (const side of ["Left", "Right"]) {
    // re-plant the feet: toes flat, facing +Z (squatters' feet stay planted)
    const ft = byName(sq, side + "Foot"),
      toe = ft.children[0];
  ...
  for (const side of ["Left", "Right"]) {
    ...
    kneeLocal[side] = k.applyQuaternion(
      byName(sq, side + "Thigh")
  ...
  for (const side of ["Left", "Right"]) {
    const th = byName(sq, side + "Thigh"),
      shB = byName(sq, side + "Shin"),
      ft = byName(sq, side + "Foot"),
      toe = ft.children[0];
    const t = th.getWorldPosition(new THREE.Vector3()),
      s = shB.getWorldPosition(new THREE.Vector3()),
    ...
    kneecap.y = 0;
    kneecap.normalize();
    const toeDir = toe.getWorldPosition(new THREE.Vector3()).sub(f);
    toeDir.y = 0;
    toeDir.normalize();
...
test("she is BIT-STILL by default — there is no idle machinery", () => {
  ...
  const snap = {};
  m.traverse((o) => {
    if (o.isBone) snap[o.name] = o.quaternion.clone();
  });
...
test("bind normalization is IMMUNE to the user's saved rotation (rig-local frame; audit P1)", () => {
  ...
  assert.equal(
    Object.keys(proc.restAdjust).length,
    0,
    "no trunk/leg normalization fires on an upright bind seen through a user rotation"
  );
});
...
test("FULLY-FOLDED binds (<35° knee) are left alone — a packaging pose is not a squat (aveline)", () => {
  ...
  m.traverse((o) => {
    if (!o.isBone) return;
    if (/^(Left|Right)Shin$/.test(o.name))
      o.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), 2.8)); // shin folded ~160° back against the thigh → knee ~20°
  });
  m.updateWorldMatrix(true, true);
  const byName = (n) => {
    let b = null;
  ...
  const ang = () => {
    const t = byName("LeftThigh").getWorldPosition(new THREE.Vector3()),
      s = byName("LeftShin").getWorldPosition(new THREE.Vector3()),
      f = byName("LeftFoot").getWorldPosition(new THREE.Vector3());
...
test("lying-bound rigs are left alone (a lying bind is a style, not a squat defect)", () => {
  ...
  model.traverse((o) => {
    if (!o.isBone) return;
    if (/^(Left|Right)Shin$/.test(o.name))
      o.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), 1.9)); // knees folded — would trip the squat gate
  });
  model.updateWorldMatrix(true, true);
  const proc = buildProceduralRig(model, {});
  assert.equal(
    Object.keys(proc.restAdjust).length,
    0,
    "no normalization on a lying bind (legs would have folded 90deg off the body)"
  );
});
